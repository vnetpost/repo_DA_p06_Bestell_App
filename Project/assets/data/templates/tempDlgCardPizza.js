...
export function renderTmpDlgCardPizza(id) { // id -> 102, 502, ...
    let targetInst;
                ...
                <section class="cSecDlgVisiblePart">
                    <div class="cDlgMedias">
                        ...
                        <button id="cDlgCardCloseBtn" type="button">
                            <img class="cDlgCardCloseIcon" src="./assets/images/icons/close.svg" alt="close Icon">
                        </button>
                    </div>
                    ...
                    <div class="cDlgHeader">
                        ...
                        <b>from <span style="font-weight: 400;">${targetInst.getPrice("small")} €</span></b>
    ...
    function renderPizzaOptions() {
        let htmlText = "";
        targetInst.size.forEach(size => { // size -> "small, Ø 32 cm"
            const firstWord = size.split(",")[0].trim();
            const elId = `idInputPizza${capitalizeFirst(firstWord)}${targetInst.id}`;
            const priceTag = targetInst.getPrice(firstWord);
            const elValue = `${targetInst.id}_${targetInst.name}_${size}_${priceTag}`; // value="101_Salami_small, Ø 32 cm"
    ...
    // Catch Infos from Target-Instance
    function renderPizzaIngredientsOptions() {
        let htmlText = "";
        targetInst.toppingsList.forEach(topInst => {
            const elId = `idInputTop${topInst.id}`;
            const priceTag = topInst.getPrice();
            const elValue = `${topInst.id}_${topInst.name}_${topInst.size}_${priceTag}`;
            htmlText += `
                <div class="cOptions">
                    <label for="${elId}">
                        <input id="${elId}" value="${elValue}" type="checkbox" name="toppings" >
                        <span>With ${topInst.name}</span>
                    </label>
                    <b>${priceTag.replace(".", ",")} €</b>
                </div>
            `;
    ...
    function renderPizzaDrinksOptions() {
        let htmlText = "";
